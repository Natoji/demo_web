        ...
        <template v-if="user">
          ...
          <div class="relative">
            ...
            >
              ...
              <button 
                @click="handleLogout" 
                class="block w-full text-left px-4 py-2 hover:bg-rose-100 transition-colors"
              >
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const updateCartCount = async () => {
      if (props.user) {
        try {
          console.log('Updating cart count for user:', props.user.id);
          const response = await cartAPI.getCartCount(props.user.id);
          console.log('Cart count response:', response.data);
          if (response.data && typeof response.data.count === 'number') {
            cartCount.value = response.data.count;
            // Lưu cart count và user info vào localStorage
            localStorage.setItem(`cartCount_${props.user.id}`, response.data.count);
        ...
        if (savedUser) {
          try {
            ...
            if (response.data && typeof response.data.count === 'number') {
              cartCount.value = response.data.count;
              localStorage.setItem(`cartCount_${user.id}`, response.data.count);
            }
          } catch (error) {
            console.error('Error restoring cart count:', error);
            localStorage.removeItem('currentUser');
            localStorage.removeItem(`cartCount_${JSON.parse(savedUser).id}`);
          }
        } else {
          cartCount.value = 0;
        }
    ...
    onMounted(() => {
      ...
      if (savedUser) {
        try {
          ...
          if (savedCount !== null) {
            console.log('Restoring cart count from localStorage:', savedCount);
